...
class ChartDetailDialog(QDialog):
    def __init__(self, chart_type, title, data, is_dark=True, parent=None):
        ...
        # Grafik (Büyük)
        chart_frame = QFrame()
        chart_frame.setStyleSheet(f"background-color: {bg_card}; border-radius: 12px; border: 1px solid {border_color};")
...
class DashboardPage(QWidget):
    ...
    def open_chart_detail(self, chart_widget):
        dialog = ChartDetailDialog(chart_widget.chart_type, chart_widget.title, chart_widget.data, is_dark=chart_widget.is_dark, parent=self)
        dialog.exec()
