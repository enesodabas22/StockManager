...
class SimpleChartWidget(QWidget):
    ...
    def draw_pie_chart(self, painter, rect):
        # Pasta grafiği kodunda değişiklik gerekmez, etiketler zaten resmin üstüne beyaz yazılıyor
        # veya yanına yazılıyorsa self.text_color kullanılabilir.
        # Mevcut kodunuzda etiketler 'white' olarak hardcoded kalabilir (pasta dilimi koyu olduğu için).
        super().draw_pie_chart(painter, rect)
...
class AnaPencere(QMainWindow):
    ...
    def init_menu_actions(self):
        ...
        self.tema_action = QAction("Açık Temaya Geç", self)
        self.tema_action.triggered.connect(self.tema_degistir)
        self.ayarlar_menu.addAction(self.tema_action)
    ...
    def detayli_excel_raporu_olustur(self):
        ...
        try:
            ...
            # Varsayılan boş sayfayı kaldıralım (Temiz başlangıç)
...
def stil_olustur(koyu_tema=True):
    ...
    if koyu_tema:
        ...
        c = {
            ...
            "bg_sidebar": "#ffffff",  # Beyaz Sidebar
    ...
    * {{ font-family: 'Segoe UI', 'Roboto', sans-serif; color: {c['text_main']}; selection-background-color: {c['accent']}; selection-color: #ffffff; }}
    ...
    QFrame#sidebarFrame QLabel {{ color: {c['text_muted']}; font-size: 11px; font-weight: 700; text-transform: uppercase; padding-left: 12px; padding-top: 20px; letter-spacing: 0.5px; }}
    ...
    QListWidget#sidebarNav::item {{ padding: 12px 16px; border-radius: 8px; color: {c['text_muted']}; font-weight: 500; margin: 4px 12px; transition: all 0.2s; }}
    ...
    QPushButton#sourceButton {{ background: {c['bg_sidebar']}; border: 1px solid {c['border']}; color: {c['text_muted']}; padding: 12px; text-align: left; border-radius: 8px; font-size: 12px; font-weight: 500; margin: 10px; }}
    ...
    QLabel#metricValue {{ font-size: 28px; font-weight: 700; color: {c['text_main']}; padding: 4px 0; }}
    QLabel#metricValue[lowStock="true"] {{ color: {c['danger']}; }}
    QLabel#metricUnit {{ font-size: 12px; color: {c['text_muted']}; margin-left: 4px; padding-bottom: 4px; }}
    ...
    QHeaderView::section {{ background-color: {c['table_header']}; padding: 12px 10px; border: none; border-bottom: 2px solid {c['border']}; font-weight: 600; color: {c['text_muted']}; text-transform: uppercase; font-size: 12px; }}
...
if __name__ == "__main__":
    ...
    app.setStyleSheet(stil_olustur(True))
