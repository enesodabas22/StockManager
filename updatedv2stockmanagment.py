...
class StokUygulamasiWidget(QWidget):
    ...
    def arayuz_olustur(self):
        ...
        self.stok_tablosu.setEditTriggers(QAbstractItemView.NoEditTriggers)
    ...
    def stogu_guncelle_arayuz(self):
        ...
        for satir, (id_val, ad, kategori, fiyat, miktar, min_stok) in enumerate(urun_listesi):
            ...
            min_stok_item = QTableWidgetItem(str(min_stok))
    ...
    def guncelle_menusu_goster(self, satir_indeksi, buton):
        ...
        urun_id = self.stok_tablosu.item(satir_indeksi, 0).data(Qt.UserRole)
        urun_adi_gosterim = self.stok_tablosu.item(satir_indeksi, 1).text()

        menu = QMenu(self)

        # Modern menu actions with emojis
        duzenle = QAction("✏️ Bilgileri Düzenle", self)
    ...
    def show_filter_dialog(self):
        ...
        QMessageBox.information(self, "Filtreleme", "Gelişmiş filtreleme özelliği yakında eklenecek!")
    ...
    def tabloyu_filtrele(self, metin):
        ...
        for i in range(self.stok_tablosu.rowCount()):
            ...
            fiyat = self.stok_tablosu.item(i, 3).text().lower()

            matches = (metin_lower in urun_adi or
                       metin_lower in kategori or
                       metin_lower in fiyat)

            self.stok_tablosu.setRowHidden(i, not matches)
