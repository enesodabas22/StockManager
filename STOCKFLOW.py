...
from PyQt6.QtCore import Qt, pyqtSignal, QPoint, QDate, QRectF, QSize, QLocale, QRegularExpression, pyqtProperty, QPropertyAnimation, QEasingCurve
...
class UrunDuzenlemeDialog(QDialog):
    def __init__(self, urun_detaylari, parent=None):
        ...
        self.yeni_birim_input.addItems(["adet", "kg", "ton", "litre", "paket", "kutu", "koli", "çuval", "palet"])
...
class KarZararSayfasi(QWidget):
    ...
    def arayuz_olustur(self):
        ...
        filtre_layout.addWidget(self.export_btn)
    ...
    def export_to_excel(self):
        path, _ = QFileDialog.getSaveFileName(self, "Excel/CSV Olarak Kaydet", "", "CSV Dosyası (*.csv)")
        if not path: return
        try:
            ...
            QMessageBox.information(self, "Başarılı", "Tablo verileri başarıyla kaydedildi.")
        except Exception as e:
...
class SatisRaporuSayfasi(QWidget):
    ...
    def arayuz_olustur(self):
        ...
        ust_panel.addWidget(self.export_btn)
...
class StokHareketSayfasi(QWidget):
    ...
    def arayuz_olustur(self):
        ...
        ust_bar.addStretch()
        ust_bar.addWidget(self.export_btn)
    ...
    def export_to_excel(self):
        ...
        try:
            with open(path, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f, delimiter=';')
                headers = [self.rapor_tablosu.horizontalHeaderItem(i).text() for i in range(self.rapor_tablosu.columnCount())]
                writer.writerow(headers)
                for row in range(self.rapor_tablosu.rowCount()):
                    row_data = []
                    for col in range(self.rapor_tablosu.columnCount()):
                        item = self.rapor_tablosu.item(row, col)
                        row_data.append(item.text() if item else "")
                    writer.writerow(row_data)
            QMessageBox.information(self, "Başarılı", "Hareket geçmişi başarıyla kaydedildi.")
        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Kaydedilemedi:\n{e}")
...
class SimpleChartWidget(QWidget):
    ...
    def __init__(self, chart_type="line", data=None, title="", parent=None):
        ...
        # Reverted transparency to ensure widget handles its own clicks if needed
    ...
    def get_colors(self):
        if self.dark_mode:
            return {
                "text": QColor("#ffffff"), # White for visibility
                "grid": QColor("#3f3f46"), # Zinc-700
                "axis": QColor("#52525b"), # Zinc-600
                "hover_bg": QColor(255, 255, 255, 30),
                "hover_text": QColor("#ffffff")
            }
        else:
            return {
                "text": QColor("#334155"), # Slate-700
                "grid": QColor("#cbd5e1"), # Slate-300
                "axis": QColor("#94a3b8"), # Slate-400
                "hover_bg": QColor(255, 255, 255, 230),
                "hover_text": QColor("#334155")
            }

    def mousePressEvent(self, event):
        self.clicked.emit()
        event.accept() # Stop propagation to parent (ClickableCard) to prevent double firing
    ...
    def draw_bar_chart(self, painter, rect):
        ...
        for i, val in enumerate(values):
            ...
            # Etiket
            painter.setFont(QFont("Segoe UI", 8))
            painter.setPen(self.get_colors()["text"])
    ...
    def draw_pie_chart(self, painter, rect):
        ...
        for i, val in enumerate(values):
            ...
            if percentage > 0.05:
                ...
                # Yazıyı çiz
                # Fix: Use dark text for light mode, white for dark mode
                text_color = QColor("white") if self.dark_mode else QColor("#1e293b")
...
class ChartDetailDialog(QDialog):
    def __init__(self, chart_type, title, data, is_dark_theme=False, parent=None):
        super().__init__(parent)
        self.setWindowTitle(f"Detay: {title}")
        self.setMinimumSize(900, 600)
        self.is_dark_theme = is_dark_theme

        # Set Dialog Background manually to match theme if needed, though global stylesheet should handle it.
        # But to be safe and match the main window's exact navy:
        if self.is_dark_theme:
             self.setStyleSheet("background-color: #222b45; color: white;")
        else:
        ...
        # Başlık
        header = QLabel(title)
        header.setStyleSheet("font-size: 24px; font-weight: bold; border: none;")
        layout.addWidget(header)

        # Grafik (Büyük)
        chart_frame = QFrame()
        chart_frame.setObjectName("chartDetailFrame")
        chart_layout = QVBoxLayout(chart_frame)
        self.chart = SimpleChartWidget(chart_type, data, "")  # Başlık zaten yukarıda
        self.chart.setMinimumHeight(400)
        
        # *** KEY FIX: Propagate theme to the new chart instance ***
        self.chart.set_theme(self.is_dark_theme)
        ...
        # Table Styling for Dark Mode visibility
        if self.is_dark_theme:
            self.table.setStyleSheet("""
                QTableWidget { background-color: #1e293b; color: white; border: 1px solid #334155; gridline-color: #334155; }
                QHeaderView::section { background-color: #0f172a; color: white; border: 1px solid #334155; }
            """)
        else:
             self.table.setStyleSheet("""
                QTableWidget { background-color: white; color: black; border: 1px solid #cbd5e1; }
                QHeaderView::section { background-color: #f1f5f9; color: #334155; border: 1px solid #cbd5e1; }
            """)
...
class ToggleSwitch(QCheckBox):
    def __init__(self, parent=None):
        ...
        self.setCursor(Qt.CursorShape.PointingHandCursor)
        self._knob_position = 3.0 # Start Left (Light)
        
        self.animation = QPropertyAnimation(self, b"knob_position")
        self.animation.setDuration(300)
        self.animation.setEasingCurve(QEasingCurve.Type.OutBack) # Bouncy effect
        self.stateChanged.connect(self.start_animation)

    def get_knob_position(self): return self._knob_position
    ...
    def start_animation(self, state):
        self.animation.stop()
        if self.isChecked():
            self.animation.setEndValue(self.width() - self.height() + 3.0)
        else:
    ...
    def paintEvent(self, event):
        ...
        if self.isChecked():
            ...
            knob_color = QColor("#3b82f6") # Blue Moon
            icon = "🌙"
        else:
            bg_color = QColor("#e2e8f0")
            border_color = QColor("#94a3b8")
            knob_color = QColor("#f59e0b") # Orange Sun
            icon = "☀️"
        ...
        knob_size = rect.height() - 10
        # Use property for X
        knob_rect = QRectF(self._knob_position, 5, knob_size, knob_size)
        ...
        painter.drawText(knob_rect, Qt.AlignmentFlag.AlignCenter, icon) 
...
class DashboardPage(QWidget):
    ...
    def __init__(self, veritabani_yoneticisi, parent=None):
        super().__init__(parent)
        self.veritabani = veritabani_yoneticisi
        self.is_dark_theme = False # Default state to prevent AttributeError
        self.setup_ui()
        self.refresh_data()
    ...
    def create_info_card(self, title, value, color_code, target_tab_index=None):
        if target_tab_index is not None:
            ...
            frame.setProperty("clickable", "true") # Use string for sensitive QSS matching
        else:
            frame = QFrame()
            frame.setProperty("clickable", "false")

        # Map colors to ID suffixes
        if "#3b82f6" in color_code: suffix = "_Blue"
        elif "#ef4444" in color_code: suffix = "_Red"
        elif "#10b981" in color_code: suffix = "_Green"
        else: suffix = "" 
        
        frame.setObjectName(f"metricCard{suffix}")
        # Removed manual setStyleSheet to allow global theme background to apply!
    ...
    def update_card_theme(self, is_dark):
        self.is_dark_theme = is_dark # Update internal state so popups know the theme
        bg_color = "#324060" if is_dark else "#ffffff" # Lighter Navy (less grey)
        border_color = "#4b5e80" if is_dark else "#94a3b8"
        text_color = "#ffffff" if is_dark else "#0f172a"
        title_color = "#cbd5e1" if is_dark else "#64748b"
        hover_bg = "#26304a" if is_dark else "#f1f5f9" # Dynamic Hover
        
        cards = [self.card_total_stock, self.card_low_stock, self.card_monthly_sales]
        ...
        for card in cards:
            card.setStyleSheet(f"""
                ...
                QLabel {{ border: none; background: transparent; }} /* Remove borders from text */
                QLabel#metricValue {{ color: {text_color}; }}
                QLabel#metricTitle {{ color: {title_color}; }}
        ...
        # Also update chart frames
        chart_frames = [self.category_chart_frame, self.value_chart_frame, self.sales_chart_frame]
    ...
    def wrap_chart(self, chart_widget):
        # Use ClickableCard wrapper to catch clicks on padding/frame
        frame = ClickableCard()
        frame.setObjectName("chartCard")
        
        # IMPORTANT: Forward the frame's click to the chart's signal
        # This handles clicks on the border/padding
        frame.clicked.connect(chart_widget.clicked.emit)
    ...
    def open_chart_detail(self, chart_widget):
        dialog = ChartDetailDialog(chart_widget.chart_type, chart_widget.title, chart_widget.data, self.is_dark_theme, self)
        dialog.exec()
...
class AnaPencere(QMainWindow):
    ...
    def __init__(self, kullanici_adi, veritabani_yoneticisi):
        ...
        self.kullanici_adi = kullanici_adi
        self.veritabani = veritabani_yoneticisi
        self.is_dark_theme = False # Default Light
    ...
    def init_ui(self):
        ...
        # 3. Listenin tüm öğeleri alacak kadar yüksek olmasını sağlıyoruz (Piksel cinsinden)
        # 6 öğe var + paddingler için 400px yetersiz kalıyor ve scroll oluyor.
        # 600px yaparak scroll olmasını engelliyoruz, böylece en üstteki öğe kaybolmuyor.
        self.nav_list.setMinimumHeight(600)
        ...
        toggle_wrapper = QFrame()
        toggle_wrapper.setObjectName("toggleWrapper") 
        # Optional styling for wrapper if needed, or just layout
        
        self.theme_toggle = ToggleSwitch()
        self.theme_toggle.stateChanged.connect(self.toggle_theme)
        
        # Label to indicate mode? Or just the switch
        # User asked for "just add a button like this", switch implies self-explanatory
        
        sidebar_layout.addWidget(self.theme_toggle, 0, Qt.AlignmentFlag.AlignCenter) # Center the switch
    ...
    def toggle_theme(self):
        # 1. Capture Screen for Transition
        overlay = ThemeTransitionOverlay(self.centralWidget())
        ...
        # Update Charts and Cards
        if hasattr(self, 'dashboard_sayfasi'):
            # Update SimpleChartWidgets internal colors
            for widget in self.dashboard_sayfasi.findChildren(SimpleChartWidget):
                widget.set_theme(self.is_dark_theme)
            
            # Update Card Backgrounds Manually to FORCE change
            self.dashboard_sayfasi.update_card_theme(self.is_dark_theme)
...
LIGHT_THEME = """
    /* Global Reset & Fonts */
    * { font-family: 'Segoe UI', 'Inter', 'Roboto', sans-serif; selection-background-color: #3b82f6; selection-color: #ffffff; }

    /* Main Backgrounds - SLATE-300 (Darker Grey as requested) */
    QMainWindow, QWidget#central_widget { background-color: #cbd5e1; } /* Slate-300 */
    
    /* Login Window - DARK to ensure Logo visibility */
    QDialog, QWidget#authWindow { background-color: #0f172a; border: 1px solid #334155; border-radius: 12px; }
    /* Login Labels - Light Color */
    QWidget#authWindow QLabel { color: #f1f5f9; }
    
    QMessageBox { background-color: #f8fafc; color: #0f172a; }

    /* Sidebar - DARK */
    QFrame#sidebarFrame { background-color: #1e293b; border-right: 1px solid #334155; }
    QFrame#sidebarFrame QLabel { color: #94a3b8; font-size: 11px; font-weight: 700; text-transform: uppercase; padding-left: 12px; padding-top: 24px; letter-spacing: 0.8px; }
    ...
    /* Navigation List - HUGE SPACING */
    QListWidget#sidebarNav { border: none; background: transparent; outline: none; margin-top: 20px; }
    /* Fix: Force TEXT COLOR to WHITE (#ffffff) for unselected items because sidebar is Dark Navy even in Light Mode */
    QListWidget#sidebarNav::item { padding: 12px 18px; border-radius: 8px; color: #ffffff; font-weight: 500; font-size: 14px; margin: 10px 16px; border: 1px solid transparent; }
    QListWidget#sidebarNav::item:hover { background-color: #334155; color: #f1f5f9; }
    QListWidget#sidebarNav::item:selected { background-color: #3b82f6; color: #ffffff; font-weight: 600; border: 1px solid #3b82f6; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }

    /* Buttons */
    QPushButton { background-color: #2563eb; color: white; border: none; border-radius: 8px; padding: 10px 20px; font-weight: 600; font-size: 13px; }
    QPushButton:hover { background-color: #1d4ed8; }
    QPushButton:pressed { background-color: #1e40af; }
    QPushButton:disabled { background-color: #cbd5e1; color: #94a3b8; }

    QPushButton#sourceButton { background: #0f172a; border: 1px solid #334155; color: #94a3b8; padding: 12px; text-align: left; border-radius: 8px; font-size: 13px; font-weight: 500; margin: 16px; }
    
    /* Header Buttons */
    QPushButton#headerButton { background: transparent; border: 1px solid #cbd5e1; color: #475569; font-weight: 500; padding: 8px 16px; border-radius: 6px; }
    QPushButton#headerButton:hover { background: #f1f5f9; color: #1e293b; border-color: #64748b; }

    QPushButton#filterBtn { background: #ffffff; border: 1px solid #94a3b8; color: #475569; }
    QPushButton#filterBtn:hover { background: #f8fafc; border-color: #64748b; }
    QPushButton#filterBtn[filtered="true"] { background: #3b82f6; color: white; border: none; }

    QPushButton#yeniUrunBtn { background-color: #10b981; }
    QPushButton#yeniUrunBtn:hover { background-color: #059669; }

    QPushButton#menuButton { background: transparent; border: 1px solid transparent; border-radius: 4px; font-weight: 900; font-size: 18px; color: #cbd5e1; padding: 0px 8px; margin: 0; line-height: 10px; }
    QPushButton#menuButton:hover { background: #334155; color: white; }

    QPushButton#linkButton { background: transparent; color: #3b82f6; text-decoration: none; font-weight: 500; padding: 4px; }
    QPushButton#linkButton:hover { color: #60a5fa; text-decoration: underline; }

    /* Header & Content */
    QFrame#contentContainer { background: #cbd5e1; /* Slate-300 */ }
    QFrame#headerBar { background-color: #cbd5e1; border-bottom: 1px solid #94a3b8; padding: 10px 0; }
    QLabel#pageTitle { font-size: 26px; font-weight: 700; color: #1e293b; padding: 0; letter-spacing: -0.5px; }
    QLabel#pageSubtitle { font-size: 14px; color: #475569; }
    ...
    QFrame[objectName^="metricCard"] {
        ...
        border-radius: 16px;
        padding: 12px; /* Reduced padding to fit text */
        min-height: 80px;
    }
    /* Specific Border Colors */
    QFrame#metricCard_Blue { border-left: 4px solid #3b82f6; }
    QFrame#metricCard_Red { border-left: 4px solid #ef4444; }
    QFrame#metricCard_Green { border-left: 4px solid #10b981; }

    /* Hover */
    QFrame[objectName^="metricCard"][clickable="true"]:hover {
        border-color: #475569;
    ...
    /* Chart Cards */
    QFrame#chartCard { background-color: #ffffff; border-radius: 16px; border: 1px solid #94a3b8; }
    QFrame#chartCard:hover {
        border-color: #475569; /* Dark Grey (Monochrome) */
        background-color: #cbd5e1;
        cursor: pointer;
    }
    ...
    /* Labels */
    QLabel { color: #334155; }
    QLabel#metricTitle { font-size: 12px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.8px; }
    QLabel#metricTitle { font-size: 12px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.8px; }
    QLabel#metricValue { font-size: 24px; font-weight: 700; color: #0f172a; padding: 4px 0; letter-spacing: -0.5px; }
    QLabel#metricValue[lowStock="true"] { color: #ef4444; }
    QLabel#metricUnit { font-size: 13px; color: #64748b; margin-left: 4px; padding-bottom: 6px; }

    /* Search & Inputs */
    QFrame#searchContainer { background-color: #ffffff; border: 1px solid #94a3b8; border-radius: 10px; }
    QLineEdit#searchInput { border: none; background: transparent; padding: 12px; font-size: 14px; color: #0f172a; }

    QLineEdit, QComboBox, QDateEdit { background-color: #ffffff; border: 1px solid #94a3b8; border-radius: 8px; padding: 10px; color: #0f172a; font-size: 14px; }
    QLineEdit:focus, QComboBox:focus, QDateEdit:focus { border-color: #3b82f6; background-color: #ffffff; }
    QLineEdit::placeholder { color: #94a3b8; }

    /* Tables - Restore Grid Lines */
    QTableWidget { background-color: #ffffff; border: 1px solid #94a3b8; border-radius: 12px; gridline-color: #cbd5e1; outline: none; }
    QTableWidget::item { padding: 12px 12px; border-bottom: 1px solid #e2e8f0; color: #334155; }
    QTableWidget::item:selected { background-color: #eff6ff; color: #1e40af; }
    QHeaderView::section { background-color: #f1f5f9; padding: 4px; border: none; border-bottom: 2px solid #94a3b8; border-right: 1px solid #cbd5e1; font-weight: 600; color: #475569; text-transform: uppercase; font-size: 12px; letter-spacing: 0.5px; }
    QTableWidget QTableCornerButton::section { background-color: #f1f5f9; border: none; }

    /* Menus */
    QMenu { background: #ffffff; border: 1px solid #cbd5e1; padding: 6px; border-radius: 12px; }
    QMenu::item { padding: 8px 32px 8px 16px; border-radius: 6px; color: #334155; font-weight: 500; }
    QMenu::item:selected { background: #eff6ff; color: #1e40af; }
    QMenu::separator { height: 1px; background: #e2e8f0; margin: 6px 0; }

    /* ScrollArea & Dashboard Content */
    QScrollArea { background-color: transparent; border: none; }
    QWidget#dashboardContent { background-color: transparent; }

    /* Scrollbars */
    QScrollBar:vertical { border: none; background: transparent; width: 10px; margin: 0; }
    QScrollBar::handle:vertical { background: #94a3b8; min-height: 30px; border-radius: 5px; }
    QScrollBar::handle:vertical:hover { background: #64748b; }
    QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0px; }
    QScrollBar::add-page:vertical, QScrollBar::sub-page:vertical { background: none; }

    QScrollBar:horizontal { border: none; background: transparent; height: 10px; margin: 0; }
    QScrollBar::handle:horizontal { background: #94a3b8; min-width: 30px; border-radius: 5px; }
    QScrollBar::handle:horizontal:hover { background: #64748b; }
    QScrollBar::add-line:horizontal, QScrollBar::sub-line:horizontal { width: 0px; }

    /* Status Bar */
    QStatusBar { background: #1e293b; border-top: 1px solid #334155; color: #94a3b8; padding: 8px; }
"""

DARK_THEME = """
    /* Global Reset & Fonts */
    * { font-family: 'Segoe UI', 'Inter', 'Roboto', sans-serif; color: #e2e8f0; selection-background-color: #3b82f6; selection-color: #ffffff; }

    /* Main Backgrounds - TRUE NAVY (Cosmic) */
    /* Main Backgrounds - TRUE NAVY (Cosmic) */
    QMainWindow, QWidget#central_widget, QScrollArea, QWidget#dashboardContent, QFrame#contentContainer, QWidget#dashboardContent > QWidget { background-color: #222b45; border: none; }
    QDialog, QWidget#authWindow { background-color: #2e3a59; border: 1px solid #101426; border-radius: 12px; }
    QMessageBox { background-color: #2e3a59; color: #ffffff; }

    /* Sidebar - DARKER NAVY */
    QFrame#sidebarFrame { background-color: #192038; border-right: 1px solid #101426; } 
    QFrame#sidebarFrame QLabel { color: #8f9bb3; font-size: 11px; font-weight: 700; text-transform: uppercase; padding-left: 12px; padding-top: 24px; }
    QLabel#logoText { font-size: 22px; font-weight: 800; color: #ffffff; }

    /* Navigation - WIDE SPACING MATCH */
    QListWidget#sidebarNav { background: transparent; outline: none; border: none; margin-top: 20px; }
    QListWidget#sidebarNav::item { padding: 12px 18px; color: #8f9bb3; margin: 10px 16px; border-radius: 8px; font-size: 14px; }
    QListWidget#sidebarNav::item:hover { background-color: #2e3a59; color: #ffffff; }
    QListWidget#sidebarNav::item:selected { background-color: #3366ff; color: white; }

    /* Buttons */
    QPushButton { background-color: #3366ff; color: white; border: none; border-radius: 8px; padding: 10px 20px; font-weight: 600; font-size: 13px; }
    QPushButton:hover { background-color: #274bdb; }
    QPushButton#sourceButton { background: #222b45; border: 1px solid #101426; color: #8f9bb3; padding: 12px; text-align: left; border-radius: 8px; font-size: 13px; font-weight: 500; margin: 16px; }
    QPushButton#headerButton { background: transparent; border: 1px solid #2e3a59; color: #8f9bb3; }
    QPushButton#headerButton:hover { background: #2e3a59; border-color: #3366ff; color: #ffffff; }

    /* Inputs & ComboBox */
    QLineEdit, QComboBox, QDateEdit { background-color: #2e3a59; border: 1px solid #101426; border-radius: 8px; padding: 10px; color: #ffffff; }
    QLineEdit:focus, QComboBox:focus { border-color: #3366ff; }
    
    QComboBox::drop-down { border: none; }
    QComboBox::down-arrow { image: none; border-left: 2px solid #8f9bb3; border-bottom: 2px solid #8f9bb3; width: 8px; height: 8px; margin-right: 10px; transform: rotate(-45deg); }
    ...
    /* Cards - Distinct Lighter Navy */
    /* Cards - Distinct Lighter Navy - FORCED LIGHTER & EXPLICIT IDs */
    QFrame#chartCard, QFrame#metricCard, QFrame[objectName^="metricCard"] { 
        background-color: #324060; 
        border: 1px solid #4b5e80; 
        border-radius: 16px; 
    }
    /* Fix for Label Borders inside Cards */
    QFrame[objectName^="metricCard"] QLabel { border: none; background: transparent; }
    QFrame[objectName^="metricCard"][clickable="true"]:hover, QFrame#chartCard:hover { border-color: #3b82f6; background-color: #26304a; cursor: pointer; }
    ...
    QLabel#metricValue { color: #ffffff; font-size: 24px; font-weight: 700; }
    QLabel#metricTitle { color: #8f9bb3; font-weight: 600; }

    /* Tables */
    QTableWidget { background-color: #334155; border: 1px solid #101426; gridline-color: transparent; }
    QTableWidget::item { border-bottom: 1px solid #192038; color: #edf1f7; padding: 12px; }
    QHeaderView::section { background-color: #222b45; border-bottom: 2px solid #101426; color: #8f9bb3; }
"""
...
if __name__ == "__main__":
    ...
    if 'LIGHT_THEME' in globals():
        app.setStyleSheet(LIGHT_THEME)
    elif 'STYLESHEET' in globals():
